...
func TestLatestBackupFilePathSelection(t *testing.T) {
	...
	t.Run("skips invalid newer backups", func(t *testing.T) {
		...
		writeBackupPayload(
			...
			validBackupPayload(testTokenID(0x04), newer+1),
		)

		wrongNetworkID := testTokenID(0x05)
		wrongNetworkPayload := validBackupPayload(wrongNetworkID,
			newer+2)
	...
	t.Run("tie breaks by token id", func(t *testing.T) {
		...
		lowerPath := writeBackupPayload(
			...
			validBackupPayload(lowerID, timestamp),
		)
		higherPath := writeBackupPayload(
			t, svc, dir, higherID, timestamp,
			validBackupPayload(higherID, timestamp),
		)
...
func TestRecoverDepositParsesAndDelegates(t *testing.T) {
	...
	require.EqualValues(t, 5, depositMgr.recoverReq.ScanLimit)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			writeBackupPayload(
				...
				validBackupPayload(backupTokenID, 2),
			)
...
func TestRestoreRejectsInvalidPayloadMetadata(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			payload := validBackupPayload(tokenID, 1)
			test.mutate(payload)
...
func validBackupPayload(tokenID string, tokenCreatedAt int64) *backupPayload {
...
func writeTokenFile(t *testing.T, path string, seed byte, createdAt time.Time,
	...
	err = os.WriteFile(path, serialized.Bytes(), 0600)
	require.NoError(t, err)
