...
func latestBackupFilePath(dataDir string, key [32]byte,
	...
	for _, entry := range dirEntries {
		...
		if payload.L402TokenID != details.tokenID {
			// The filename token ID is part of the immutable generation
			// identity, so mismatches are treated as invalid candidates.
			if firstErr == nil {
				firstErr = fmt.Errorf("backup file %s token id "+
					"mismatch", path)
			}
			continue
		}
