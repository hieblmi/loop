...
func (m *Manager) OpenChannel(ctx context.Context,
	...
	chanFundingAmt, _, calcErr := withdraw.CalculateWithdrawalTxValaues(
	...
	chanTxHash, err := m.openChannelPsbt(ctx, openChanRequest, deposits)
	if err != nil {
...
func (m *Manager) openChannelPsbt(ctx context.Context,
	...
	deposits []*deposit.Deposit) (*chainhash.Hash, error) {
	...
	for {
		...
		case *lnrpc.OpenStatusUpdate_PsbtFund:
			...
			signedTx, unsignedPsbt, err := m.cfg.WithdrawalManager.CreateFinalizedWithdrawalTx(
				ctx, deposits, channelFundingAddress,
				int64(req.SatPerVbyte), fundingAmount,
			)
			if err != nil {
