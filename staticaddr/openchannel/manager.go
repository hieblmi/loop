...
func (m *Manager) OpenChannel(ctx context.Context,
	...
	if req.MinConfs != 0 && req.MinConfs != defaultUtxoMinConf {
		return nil, fmt.Errorf("custom MinConfs not supported for " +
			"PSBT funding, only the default is allowed")
	}

	if req.SpendUnconfirmed {
		return nil, fmt.Errorf("SpendUnconfirmed is not supported " +
			"for PSBT funding")
	}

	if req.MinConfs != defaultUtxoMinConf {
		return nil, fmt.Errorf("custom MinConfs not supported " +
			"for PSBT funding")
	}
