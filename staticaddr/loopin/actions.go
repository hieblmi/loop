...
func (f *FSM) MonitorInvoiceAndHtlcTxAction(ctx context.Context,
	...
	// confirmation risk was accepted. When no notification manager is
	// available, preserve the legacy fallback that starts the timer after
	// the original deposits reach the old confirmation target.
	var (
	...
	startPaymentDeadline := func(reason string) {
		...
		timeout := time.Duration(f.loopIn.PaymentTimeoutSeconds) *
			time.Second
		if timeout == 0 {
			timeout = DefaultPaymentTimeoutSeconds * time.Second
		}
	...
	for {
		...
		case currentHeight := <-blockChan:
			...
			if legacyFallbackEnabled && legacyMinConfsReached(
