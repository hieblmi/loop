...
// TestMonitorInvoiceAndHtlcTxCancelsWhenOriginalOutpointVanishes verifies that
// a missed server risk rejection is recovered by checking the immutable
// original selected deposit outpoints while the client is still waiting for
// confirmation risk acceptance.
func TestMonitorInvoiceAndHtlcTxCancelsWhenOriginalOutpointVanishes(
	t *testing.T) {
	...
	require.Equal(t, []wire.OutPoint{depositOutpoint}, txOutChecker.outpoints)
...
// TestMonitorInvoiceAndHtlcTxSkipsLegacyFallbackWithRiskNotifications verifies
// that the local legacy confirmation fallback cannot race ahead of the server's
// dynamic confirmation-risk tracker when risk notifications are available.
func TestMonitorInvoiceAndHtlcTxSkipsLegacyFallbackWithRiskNotifications(
	t *testing.T) {
