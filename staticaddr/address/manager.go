...
func (m *Manager) handleAddrRequest(managerCtx context.Context, req request) {
	...
	params, expiry, addrErr := m.newAddress(req.ctx)
	if addrErr != nil {
	...
	addr, err := params.TaprootAddress(m.cfg.ChainParams)
	if err != nil {
		log.Errorf("Unable to create taproot address: %v", err)
		sendResponse(response{err: addrErr})
...
func (m *Manager) NewAddress(ctx context.Context) (string, int64, error) {
...
func (m *Manager) newAddress(ctx context.Context) (*Parameters, int64, error) {
	...
	return addrParams, int64(serverParams.Expiry), nil
}
