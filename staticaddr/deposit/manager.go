...
func (m *Manager) reviveReappearedDeposits(ctx context.Context,
	...
	for _, candidate := range candidates {
		...
		if err != nil {
			...
			deposit.ConfirmationHeight = previousConfirmationHeight
			deposit.SetStateNoLock(previousState)
			rollbackErr := m.cfg.Store.UpdateDeposit(ctx, deposit)
			deposit.Unlock()
			if rollbackErr != nil {
				return fmt.Errorf("unable to start deposit FSM: %w; "+
					"unable to roll back revived deposit: %v",
					err, rollbackErr)
			}
