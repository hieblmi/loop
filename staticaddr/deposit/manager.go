...
func (m *Manager) RecoverDeposit(ctx context.Context,
	...
	err = m.ensureRecoveryOutpointUnspent(ctx, verified)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) ensureRecoveryOutpointUnspent(ctx context.Context,
	verified *verifiedRecoveryDeposit) error {

	if m.cfg.ChainKit == nil {
		return errors.New("chain kit client required for deposit recovery")
	}

	_, bestHeight, err := m.cfg.ChainKit.GetBestBlock(ctx)
	if err != nil {
		return fmt.Errorf("unable to get best block: %w", err)
	}
	if bestHeight < int32(verified.confirmationHeight) {
		return fmt.Errorf("best height %d is below confirmation "+
			"height %d", bestHeight, verified.confirmationHeight)
	}
	m.currentHeight.Store(uint32(bestHeight))

	for height := verified.confirmationHeight; height <= int64(bestHeight); height++ {
		block := verified.block
		if height != verified.confirmationHeight || block == nil {
			blockHash, err := m.cfg.ChainKit.GetBlockHash(ctx, height)
			if err != nil {
				return fmt.Errorf("unable to get block hash at "+
					"height %d: %w", height, err)
			}

			block, err = m.cfg.ChainKit.GetBlock(ctx, blockHash)
			if err != nil {
				return fmt.Errorf("unable to get block at "+
					"height %d: %w", height, err)
			}
		}

		if blockSpendsOutpoint(block, verified.outPoint) {
			return fmt.Errorf("deposit outpoint %v is already spent",
				verified.outPoint)
		}
	}
...
func blockSpendsOutpoint(block *wire.MsgBlock, outpoint wire.OutPoint) bool {
	if block == nil {
		return false
	}
...
func (m *Manager) matchRecoveryAddressParams(ctx context.Context,
	...
	*address.Parameters, bool, error) {
	...
	if scanLimit == 0 {
		scanLimit = DefaultRecoveryScanLimit
	}
