...
// TestTransitionDepositsRejectsReplacedDeposit verifies that a transition which
// captured an active FSM before replacement cannot move the deposit back out of
// Replaced after the replacement path releases the deposit lock.
func TestTransitionDepositsRejectsReplacedDeposit(t *testing.T) {
	ctx := context.Background()
	outpoint := wire.OutPoint{
	...
	err := manager.TransitionDeposits(
		ctx, []*Deposit{deposit}, OnLoopInInitiated, LoopingIn,
	)
...
func TestReconcileDepositsReactivatesReappearedReplacedDeposit(t *testing.T) {
	...
	require.Zero(t, deposit.ConfirmationHeight)
	require.Len(t, manager.activeDeposits, 1)
	require.Equal(t, []fsm.StateType{Replaced, Deposited}, updateStates)
}

// TestReconcileDepositsRollsBackRevivalOnFSMStartFailure verifies that a
// failed revival doesn't leave the store or memory saying a deposit is active
// without an FSM.
func TestReconcileDepositsRollsBackRevivalOnFSMStartFailure(t *testing.T) {
	ctx := context.Background()
	outpoint := wire.OutPoint{
	...
	err := manager.reconcileDeposits(ctx)
	require.ErrorContains(t, err, "unable to revive reappeared deposits")
	require.Equal(t, Replaced, deposit.GetState())
	require.Zero(t, deposit.ConfirmationHeight)
	require.Empty(t, manager.activeDeposits)
	require.Equal(
		t, []fsm.StateType{Replaced, Deposited, Replaced},
		updateStates,
	)
	require.EqualValues(t, []int64{0, 0, 0}, updateHeights)
}
